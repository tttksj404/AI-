...
SRC = ROOT / "기획서_Sentinel30_lean.md"
OUT = ROOT / "기획서_Sentinel30_v3_lean.pdf"

# NanumGothic (mac에서 reportlab 호환)
pdfmetrics.registerFont(TTFont("Malgun", str(ROOT / "fonts/NanumGothic-Regular.ttf")))
pdfmetrics.registerFont(TTFont("MalgunBold", str(ROOT / "fonts/NanumGothic-Bold.ttf")))
...
def inline(text):
    # Malgun 폰트에 없는 글리프 정리
    text = text.replace("−", "-")  # MINUS SIGN → HYPHEN-MINUS
    # Enclosed Alphanumerics (①~⑳) → (1)~(20) — Malgun에 글리프 없음
...
def build_table(header, body):
    ...
    def fix(row, header_row=False):
        ...
        style = ParagraphStyle(
            ...
            fontSize=10 if header_row else 10,
            leading=14,
            textColor=SLATE if header_row else INK,
            spaceAfter=0,
        )
    ...
    # Minimal table: cream bg, thin INK/ORANGE rules, no fill stripes.
    style = [
        ("FONTNAME", (0, 0), (-1, -1), "Malgun"),
        ("FONTNAME", (0, 0), (-1, 0), "MalgunBold"),
        ("BACKGROUND", (0, 0), (-1, -1), CREAM),
        ("TEXTCOLOR", (0, 0), (-1, 0), SLATE),  # header label dim
        ("FONTSIZE", (0, 0), (-1, -1), 10),
        ...
        # Header: orange rule above + below
        ("LINEABOVE", (0, 0), (-1, 0), 1.4, ORANGE),
        ("LINEBELOW", (0, 0), (-1, 0), 0.6, ORANGE),
    ]
    # Hairline between body rows
    for r in range(1, len(data) - 1):
        style.append(("LINEBELOW", (0, r), (-1, r), 0.4, LINE))
    # Bottom rule
    style.append(("LINEBELOW", (0, -1), (-1, -1), 0.8, INK))
...
def image_within(lines, start, max_lines=6):
    """Return True if a markdown image appears within next `max_lines` non-empty
    lines, while not crossing into a new heading (#/##/###)."""
    seen = 0
    k = start
    while k < len(lines) and seen < max_lines:
        s = lines[k].strip()
        k += 1
        if not s:
            continue
        if s.startswith("#"):
            return False  # 다음 헤딩 도달 — break 검사 종료
        if IMG_RE.fullmatch(s):
            return True
        seen += 1
    return False
...
def parse_md(md_text):
    ...
    while i < n:
        ...
        if stripped.startswith("# "):
            ...
            # Roman chapter: 전면 표지 페이지 대신 한 페이지 시작에서 큰 H1 + 태그라인 1줄
            roman = re.match(r"^([IVXLC]+)\.\s+(.+)$", title)
            # trailing Spacer/HRFlowable 제거 (다음 페이지로 밀려 빈 페이지가 생기는 것 방지)
            while flow and isinstance(flow[-1], (Spacer, HRFlowable)):
                flow.pop()
            flow.append(PageBreak())
            if roman:
                eyebrow = ParagraphStyle(
                    "Eyebrow", parent=BODY, fontName="MalgunBold", fontSize=10,
                    leading=12, textColor=ORANGE, spaceAfter=2,
                )
                tag_style = ParagraphStyle(
                    "ChapTag", parent=BODY, fontName="Malgun", fontSize=11,
                    leading=14, textColor=SLATE, spaceAfter=8,
                )
                flow.append(Paragraph(f"CHAPTER {roman.group(1)}", eyebrow))
                flow.append(Paragraph(inline(roman.group(2)), H1))
                flow.append(HRFlowable(width="40%", thickness=2, color=ORANGE,
                                       spaceBefore=0, spaceAfter=6))
                tag = SECTION_TAGLINES.get(roman.group(1), "")
                if tag:
                    flow.append(Paragraph(tag, tag_style))
            else:
            ...
            continue
        if stripped.startswith("## "):
            # 다음 6개 nonblank 라인 안에 이미지가 있으면 헤딩-이미지 분리 방지
            if image_within(lines, i + 1):
        ...
        if re.fullmatch(r"-{3,}", stripped):
            # 다음 nonblank가 `# Chapter`면 PageBreak이 이미 분리하므로 HR 생략
            # (빈 페이지에 HR만 남는 문제 방지)
            k = i + 1
            while k < len(lines) and not lines[k].strip():
                k += 1
...
def main():
    ...
    story = parse_md(md)
    # 첫 #-헤딩이 이미 PageBreak를 추가하므로 중복 PageBreak는 넣지 않음 (빈 페이지 방지)
